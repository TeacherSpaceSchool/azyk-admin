...
const Row =  React.memo(({client, item, columns, specialPriceByItem, setSpecialPriceByItem, idx, list, middleList, app}) =>{
    const {isMobileApp, organization} = app;
        ...
        <div className='tableCell' style={columns[1].style}>
            <input style={{width: '100%', outline: 'none', border: 'none', fontWeight: 500, fontFamily: 'Roboto, serif'}}
                   ...
                   onChange={(event) => {
                       ...
                       saveTimeOut.current = setTimeout(async () => await setSpecialPriceClient({client: client._id, organization, price, item: item._id}), 500)
                   }}
