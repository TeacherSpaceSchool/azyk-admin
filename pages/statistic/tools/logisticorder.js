...
const LogistiOorder = React.memo((props) => {
    ...
    let [district, setDistrict] = useState(profile.role==='агент'?{_id: profile.organization}:null);
    ...
    useEffect(() => {
        (async () => {
            ...
            if(district&&organization) {
                ...
                setOrders(
                    filter==='Заказы'?
                        await getOrdersFromDistrict({organization: organization._id, district: district._id, date})
                        :
                        await getReturnedsFromDistrict({organization: organization._id, district: district._id, date})
                )
                showLoad(false)
            }
            else {
    ...
    return (
        <App cityShow pageName='Логистика' dates checkPagination={checkPagination} filters={filters}>
            <Head>
